  ...
  static async filterAsistencias(req: Request, res: Response): Promise<any> {
    try {
      ...
      if (!filter || typeof filter !== "string") {
        return res.status(400).json({ message: "El parámetro de filtro es requerido y debe ser una cadena de texto." });
      }

      const asistencias = await asistenciaService.filterAsistencia(filter);

      if (asistencias.length === 0) {
        return res.status(404).json({ message: "No se encontraron asistencias que coincidan con el filtro." });
      }
